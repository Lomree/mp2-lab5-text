...
struct MyIterator
{
	...
	{
		return pair<int, string>(it->deep, it->data);
	}

	pair<int, string> operator->()
	{
		return pair<int, string>(it->next->deep, it->next->data);
	}
...
public:
	...
	pair<int, string> pop()
	{
		pair<int, string> p(end->deep, end->data);
		Node* temp = begin;
		while (temp->next != end)
		{
			temp = temp->next;
